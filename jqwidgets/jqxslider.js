...
(function (a) {
	...
	a.extend(a.jqx._jqxSlider.prototype, {
		defineInstance: function () {
			...
			this.tooltipFormatFunction = null;
			this.tickSize = 7;
			this.layout = "normal";
			this.rtl = false;
			this._settings = {
			...
			var h = this.max - this.min, d = c[this._getSetting("size")](), e = Math.round(h / this.ticksFrequency), b = d / e;
			c.empty();
			var j = "";
			...
			var c = b - this._track.coord()[this._getSetting("left")] + this._slider.left.width() / 2, d = c / this._track[this._getSetting("size")](), e = (this.max - this.min) * d + this.min;
			...
			var g = this.step, f = (this.max - this.min) / g, c = (this._track[this._getSetting("size")]()) / f, e = this._track.coord()[this._getSetting("left")] - this._slider.left[this._getSetting("size")]() / 2, h = {
				number: -1,
			...
			if (!this._slideEvent || !this._slideEvent.args.cancel) {
				...
				if (this.tooltip) {
					if(this.tooltipFormatFunction){
						d.attr("title", this.tooltipFormatFunction(e))
					}
					else d.attr("title", e)
				}
				if (this.input) {
			...
			var e = this._track[this._getSetting("size")](), d, g;
			if (b) {
			...
			switch (c) {
				case"theme":
					...
					b._leftButton.jqxRepeatButton({theme: d});
					b._rightButton.jqxRepeatButton({theme: d});
					break;
				case"disabled":
					if (d) {
						b.disabled = true;
