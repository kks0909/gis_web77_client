    ...
    b.extend(b.jqx._jqxGrid.prototype, {
        defineInstance: function () {
            ...
            if (this.showdefaultloadelement) {
                var d = b('<div style="z-index: 99999; margin-left: -66px; left: 50%; top: 50%; margin-top: -10px; position: relative; width: 130px; height: 33px; padding: 5px; font-family: verdana; font-size: 12px; color: #767676; border-color: #898989; border-width: 1px; border-style: solid; background: #f6f6f6; border-collapse: collapse;"><div style="float: left;"><div style="float: left; overflow: hidden; width: 32px; height: 32px;" class="jqx-grid-load"/><span style="margin-top: 10px; float: left; display: block; margin-left: 5px;" >' + this.gridlocalization.loadtext + "</span></div></div>");
        ...
        }, hiderows: function (rowsIndexesArr) {
            if (this._loading) {
                throw new Error("jqxGrid: " + this.loadingerrormessage);
                return false
            }
            if (rowsIndexesArr == undefined || rowsIndexesArr == null || rowsIndexesArr.length == 0) {
                return false
            }
            for (var i=0; i<rowsIndexesArr.length; i++) {
                var d = rowsIndexesArr[i];
                this.hiddenboundrows[d] = {index: d, hidden: true};
                rowsIndexesArr[i] = this.getrowvisibleindex(d);
            }
            return this._setrowsvisibility(rowsIndexesArr, true)
        }, showrow: function (d) {
            if (this._loading) {
                throw new Error("jqxGrid: " + this.loadingerrormessage);
                return false
        ...
        }, showrows: function (rowsIndexesArr) {
            if (this._loading) {
                throw new Error("jqxGrid: " + this.loadingerrormessage);
                return false
            }
            ...
            return this._setrowsvisibility(rowsIndexesArr, false)
            ...
            return false
        }, _setrowsvisibility: function (d, f, e) {
            ...
            var n = function () {
                ...
                var w = p.host.coord(true);
                if (p.hasTransform) {
                    w = b.jqx.utilities.getOffset(p.host);
                    t = b.jqx.utilities.getOffset(k)
                }
                if (w.left + p.host.width() > parseInt(t.left) + p.gridmenu.width()) {
                    p.gridmenu.jqxMenu("open", t.left, t.top + z)
                } else {
                    p.gridmenu.jqxMenu("open", k.width() + t.left - p.gridmenu.width(), t.top + z)
                }
                if (p.gridmenu.width() < 100) {
                    p._arrangemenu()
                }
                p._hasOpenedMenu = true;
                var x = p._getmenuitembyindex(0);
                var q = p._getmenuitembyindex(1);
                var B = p._getmenuitembyindex(2);
                var y = p._getmenuitembyindex(3);
                var r = p._getmenuitembyindex(4);
                var C = p._getmenuitembyindex(5);
                if (x != null && q != null && B != null) {
                    var u = m.sortable && p.sortable;
                    p.gridmenu.jqxMenu("disable", x.id, !u);
                    p.gridmenu.jqxMenu("disable", q.id, !u);
                    p.gridmenu.jqxMenu("disable", B.id, !u);
                    if (m.datafield) {
                        if (p.sortcolumn == m.datafield) {
                            var v = p.getsortinformation();
                            if (u) {
                                if (v.sortdirection.ascending) {
                            ...
                            p.gridmenu.jqxMenu("disable", B.id, true)
                        }
                    }
                }
                if (y != null && r != null) {
                    if (!p.groupable || !m.groupable) {
                        ...
                        if (p.groups && p.groups.indexOf(m.datafield) != -1) {
                            p.gridmenu.jqxMenu("disable", y.id, true);
                            p.gridmenu.jqxMenu("disable", r.id, false)
                        } else {
                ...
                if (C != null) {
                    ...
                    if (p.filterable && p.showfiltermenuitems) {
                        if (!m.filterable) {
                            ...
                            p.gridmenu.height(A + 180);
                            b(C).css("display", "block")
                        }
                    }
                }
        ...
        }, _iscellselected: function (i, h, f) {
            var e = false;
            var d = 0;
            if (this.virtualmode && this.pageable && this.groupable) {
            ...
            if (i && h.bounddata != null) {
                if (this.selectionmode != "singlerow") {
                    ...
                    if (this.dataview.filters.length > 0) {
                        ...
                        for (var g in this.selectedcells) {
                            if (g == d + h.bounddata.boundindex == this.selectedrowindex) {
                                e = true;
                                break
                            }
                        }
            ...
            if (i != undefined) {
                ...
                var e = function (l, k) {
                    if (l._loading) {
                        throw new Error("jqxGrid: " + l.loadingerrormessage);
                        return false
                    }
                    var m = false;
                    var n = l.vScrollInstance.value;
                    if (!b.isArray(k)) {
                        var m = l.dataview.deleterow(k)
                    } else {
                        b.each(k, function () {
                            m = l.dataview.deleterow(this, false)
                        });
                        l.dataview.refresh()
                    }
                    if (l._updating == undefined || l._updating == false) {
                        if (g == undefined || g == true) {
                            l._render(true, true, false, false);
                            if (l.vScrollBar.css("visibility") != "visible") {
                                l._arrange();
                                l._updatecolumnwidths();
                                l._updatecellwidths();
                                l._renderrows(l.virtualsizeinfo)
                            }
                        }
                    }
            ...
            if (this.hScrollBar[0].style.top != v + x - s - d - r + "px" || this.hScrollBar[0].style.left != "0px") {
                this.hScrollBar.css({top: v + x - s - d - r + "px", left: "0px"})
            }
            ...
            if (!this.autoheight) {
                ...
                if (this.vScrollBar[0].style.left != parseInt(A) - parseInt(d) - s + "px" || this.vScrollBar[0].style.top != v + "px") {
                    this.vScrollBar.css({left: parseInt(A) - parseInt(d) - s + "px", top: v})
                }
            }
            if (this.rtl) {
                this.vScrollBar.css({left: "0px", top: v});
                if (this.vScrollBar.css("visibility") != "hidden") {
                    this.hScrollBar.css({left: d + 2})
                }
            }
            ...
            if (this.showstatusbar) {
                ...
                if (this.showaggregates) {
                    if (this.hScrollBar.css("visibility") == "visible") {
                        this.hScrollBar.css({top: v + x - s - d - r + this.statusbarheight + "px"});
