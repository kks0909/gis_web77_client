...
(function (a) {
	a.jqx.jqxWidget("jqxRubrTree", "", {});
	a.extend(a.jqx._jqxRubrTree.prototype, {
		defineInstance: function () {
			...
			var d = 0;
			var c = this.panel.jqxTreePanel("getVScrollPosition");
			...
			if (this.panel) {
				if (this.width != null && this.width.toString().indexOf("%") != -1) {
					var b = this;
					this.panel.jqxTreePanel("width", "100%");
					b.removeHandler(a(window), "resize.jqxtree" + b.element.id);
					b.addHandler(a(window), "resize.jqxtree" + b.element.id, function () {
						b._calculateWidth()
					})
				} else {
					this.panel.jqxTreePanel("width", this.host.width())
				}
				this.panel.jqxTreePanel("_arrange")
			}
			...
			var c = this.panel.jqxTreePanel("getVScrollPosition");
			var e = this.panel.jqxTreePanel("getHScrollPosition");
			var b = parseInt(this.host.height());
			var f = a(d).position().top;
			if (f <= c || f >= b + c) {
				this.panel.jqxTreePanel("scrollTo", e, f - b + a(d).outerHeight())
			}
			...
			a.each(this.items, function () {
				...
				if (h != 0) {
					//FIX +200 добавлено, чтобы в скрол помещалась предположительно вся запись для мелких разрешений браузера
					//var l = this.titleElement.outerWidth() + 10 + g + (1 + this.level) * 20 + 200;
					var l = this.titleElement.outerWidth() + 10 + g + (1 + this.level) * 20
					e = Math.max(e, l);
					if (this.hasItems) {
			...
			if (f.panel) {
				if (e > this.host.width()) {
					...
					var d = f.panel.jqxTreePanel("vScrollBar").css("visibility") !== "hidden" ? 10 : 0;
					b += d;
					//f.panel.jqxTreePanel({horizontalScrollBarMax: b})
				} else {
					//f.panel.jqxTreePanel({horizontalScrollBarMax: 0})
				}
			}
			...
			this._addInput();
			if (this.host.jqxTreePanel) {
				if (this.host.find("#panel" + this.element.id).length > 0) {
					this.panel.jqxTreePanel({touchMode: this.touchMode});
					this.panel.jqxTreePanel("refresh");
					return
				}
				...
				if (a.jqx.browser.msie && a.jqx.browser.version < 8) {
					b.jqxTreePanel("content").css("left", "0px")
				}
				...
				var e = a.data(b[0], "jqxTreePanel").instance;
				if (e != null) {
			...
			if (h.toggleMode != "click") {
				h.addHandler(c, "click", function (l) {
					...
					if (h.panel != null) {
						h.panel.jqxTreePanel({focused: true})
					}
					c.focus()
				})
			}
			...
			if (this.host.jqxTreePanel) {
				this.panel.jqxTreePanel({theme: h})
			}
			...
			this.items[x] = new a.jqx._jqxRubrTree.jqxRubrTreeItem();
			...
			if (this.checkboxes) {
				if (this.host.jqxCheckBox) {
					...
					throw new Error("jqxRubrTree: Missing reference to jqxcheckbox.js.");
					return
				}
			} else {
				if (a.jqx.browser.msie && a.jqx.browser.version < 8) {
			...
			if (this.panel) {
				this.panel.jqxTreePanel("destroy");
				this.panel = null
			}
			this.host.remove()
			...
			if (l == "width" || l == "height") {
				...
				if (d.host.jqxTreePanel) {
					...
					d.panel.jqxTreePanel({sizeMode: k})
				}
			}
			if (l == "touchMode") {
			...
			if (l == "source" || l == "checkboxes") {
				if (this.source != null) {
					...
					if (!d.host.jqxTreePanel) {
						...
						d.panel.jqxTreePanel("setcontent", f)
					}
					var e = d.disabled;
...
(function (a) {
	a.jqx._jqxRubrTree.jqxRubrTreeItem = function (e, d, b) {
		var c = {
			label: null,
			id: e,
			parentId: d,
