...
(function (a) {
	a.jqx.jqxWidget("jqxVideoWindow", "", {});
	a.extend(a.jqx._jqxVideoWindow.prototype, {
		defineInstance: function () {
			...
			this.host.detach();
			a(document.body).prepend(this.host);
			var f = this;
			var g = function () {
			...
			if (!this.isModal) {
				...
				a.each(g, function (q) {
					if (this.data()) {
						if (this.data().jqxVideoWindow) {
							var o = this.data().jqxVideoWindow.instance;
			...
			while (f) {
				...
				if (a(e[f].attr("id")).length > 0) {
					if (a(e[f].attr("id")).jqxVideoWindow("isModal")) {
						return false
					}
				}
			}
			...
			if (!this.rtl) {
				this._closeButtonWrapper.css({position: "absolute", right: "0px", left: ""})
			} else {
				this._closeButtonWrapper.css({position: "absolute", left: "0px", right: ""})
			}
			...
			this.host.css({"max-height": this.maxHeight, "min-width": this.minWidth, "max-width": this.maxWidth});
			if (!e) {
				this._setChildrenLayout()
			}
			...
			if (this.position instanceof Array && this.position.length === 2 && typeof this.position[0] === "number" && typeof this.position[1] === "number") {
				this.host.css({left: this.position[0], top: this.position[1]})
			} else {
				if (this.position instanceof Object && this.position.x !== "undefined" && this.position.y != "undefined") {
					if (this.position.left) {
						this.host.offset(this.position)
					} else {
						this.host.css({left: this.position.x, top: this.position.y})
					}
				} else {
			...
			if (!this.collapsed && !this.host.is(":animated")) {
				...
				this._positionBeforeCollapse = this.host.position();
				this._heightBeforeCollapse = this.host.height();
				this._minHeightBeforeCollapse = this.host.css("min-height");
				this._widthBeforeCollapse = this.host.width();
				this._minWidthBeforeCollapse = this.host.css("min-width");
				this.collapsed = true;
				this._collapseButton.addClass(e.toThemeProperty("jqx-window-collapse-button-collapsed"));
				this._collapseButton.addClass(e.toThemeProperty("jqx-icon-arrow-down"));
				this.move(0,0);
				this.maximize(10000,10000);
			...
			if (this.collapsed && !this.host.is(":animated")) {
				...
				e.collapsed = false;
				e.resizable = true;
				var pp = e._positionBeforeCollapse;

				e.normalize(e._widthBeforeCollapse,e._heightBeforeCollapse);
				e.move(pp.left,pp.top);
				e.host.css("min-height", e._minHeightBeforeCollapse);
				e.host.css("min-width", e._minWidthBeforeCollapse);
			...
			while (f) {
				...
				if (this._toClose(h, g[f])) {
					g[f].jqxVideoWindow("closeWindow", "close");
					g.splice(f, 1)
				}
			}
			if (this._toClose(h, e)) {
				e.jqxVideoWindow("closeWindow", "close");
...
(function (b) {
		...
		return {
			...
			_validateResizeProperties: function () {
				try {
					...
					if (this.resizeParent && this.resizeParent.width && this.resizeParent.height && this.resizeParent.left && this.resizeParent.top && ((this.resizeParent.width < this.resizeTarget.width() || this.resizeParent.width < this.maxWidth) || (this.resizeParent.height < this.resizeTarget.height() || this.resizeParent.height < this.maxHeight))) {
						throw new Error(this._resizeExceptions.invalidSize)
					}
				} catch (d) {
					alert(d)
			...
			_prepareResizeMethods: function (d) {
				this._resizeMethods = [];
				if (d.indexOf("normalize") >= 0) {
					this._resizeMethods.push(this._resizeNormalize)
				}
				if (d.indexOf("left") >= 0) {
			...
			_validateHeight: function (f, k, i) {
				...
				if (i === "bottom" && (f + k.position().top + l + e > g.height + g.top)) {
					return {fix: g.height - k.position().top - l - e + g.top, result: false}
				}
				if (i === "top" && k.lastHeight - f + k.y < g.top) {
					return {fix: f + (k.lastHeight - f + k.y) - g.top, result: false}
				}
				if (f < k.minHeight) {
					return {fix: k.minHeight, result: false}
				}
				if (f > k.maxHeight) {
					return {fix: k.maxHeight, result: false}
				}
				return {result: true, fix: f}
			},
			_validateWidth: function (k, i, h) {
				...
				if (h === "right" && (k + i.position().left + l + e > f.width + f.left)) {
					return {fix: f.width - i.position().left - l - e + f.left, result: false}
				}
				if (h === "left" && (i.lastWidth - k + i.x < f.left)) {
					return {fix: k + (i.lastWidth - k + i.x) - f.left, result: false}
				}
				if (k < i.minWidth) {
					return {fix: i.minWidth, result: false}
				}
				if (k > i.maxWidth) {
					return {fix: i.maxWidth, result: false}
				}
				return {result: true, fix: k}
			},
			...
			_resizeNormalize: function (e) {
				...
				var ff = e.y;
				if (e.element.height() !== ff) {
					e.self._setResizeChildrenSize(ff, "height");
					e.element.height(ff);
					e.self._raiseResizeEvent(0)
				}
				return f
	...
	b.extend(b.jqx._jqxVideoWindow.prototype, a)
}(jQuery));